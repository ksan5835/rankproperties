    ...
    <div class="es_my_listing_search clearfix" id="search_<?php echo $search_layout?>">
        ...
        <form method="get" action="<?php echo $search_page_url; //home_url(); ?>/">
            ...
            <div class="es_my_listing_search_lower clearfix">
                ...
                <?php 
                    if ( $search_country == 1 ) {
                        es_search_input_select(__("Country", 'es-plugin'), 
                            'country', 'estatik_manager_countries');
                    }

                    if ( $search_state == 1 ) {
                        $table = ( $search_country == 1 ) ? '' : 'estatik_manager_states';
                        es_search_input_select(__("State/Region", 'es-plugin'), 
                            'state', $table);
                    }

                    if ( $search_city == 1 ) {
                        $table = ( $search_state == 1 ) ? '' : 'estatik_manager_cities';
                        es_search_input_select(__("City", 'es-plugin'), 
                            'city', $table);
                    }
                    ...
                    if ( $search_category == 1 ) {
                        es_search_input_select(__("Category", 'es-plugin'), 
                            'category', 'estatik_manager_categories');
                    }

                    if ( $search_type == 1 ) {
                        es_search_input_select(__("Type", 'es-plugin'), 
                            'type', 'estatik_manager_types');
                    }

                    if ( $search_agent == 1 ) {
                        es_search_input_select(__("Agent", 'es-plugin'), 
